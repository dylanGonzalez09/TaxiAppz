...
import 'otp_keyboard_field.dart';

class OtpPinFieldState extends State<OtpPinField>
    with TickerProviderStateMixin {
  late FocusNode _focusNode;
  late List<String> pinsInputted;
  ...
  Widget cursorWidget({Color? cursorColor, double? cursorWidth, int? index}) {
    return Container(
      padding: pinsInputted[index ?? 0].isNotEmpty
  ...
  Widget _buildFieldInput(BuildContext context, int i) {
    ...
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(color: CustomColors.clr_E2E2E2, width: 1.5)),
        child: Stack(
          children: [
            Center(
  ...
  String _getPinDisplay(int position) {
    var display = '';
    var value = pinsInputted[position];
    display = value;
    return value.isNotEmpty ? display : value;
  }

  void _bindTextIntoWidget(String text) {
    for (var i = text.length; i < pinsInputted.length; i++) {
      pinsInputted[i] = '';
    }
    if (text.isNotEmpty) {
      for (var i = 0; i < text.length; i++) {
        pinsInputted[i] = text[i];
      }
    }
  }
