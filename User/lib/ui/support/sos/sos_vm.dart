...
import '../../bottomsheets/add_sos_bs/add_sos_bs.dart';
...
class SosVm extends BaseVm {
      ...
      final response = await showModalBottomSheet(
          ...
          builder: (_) {
            return ConfirmationBs(
              title: vm.translation.txt_delete_sos,
              subTitle: vm.translation.txt_delete_sos_desc,
  ...
  //     final map = {
  //       AppConstants.country: data[AppConstants.country],
  //       AppConstants.title: data[AppConstants.title],
  //       AppConstants.phoneNumber: data[AppConstants.phoneNumber],
  //     };
  //     final country = (data[AppConstants.country] ?? '').trim();
  //     final name = (data[AppConstants.title] ?? '').trim();
  //     final phone = (data[AppConstants.phoneNumber] ?? '').trim();
  //     final isDuplicate = sosList?.any((e) {
  //           final existingName = (e.title ?? '').trim().toLowerCase();
  //           final existingPhone =
  //               (e.phoneNumber ?? '').replaceAll(RegExp(r'\D'), '');
  //           return existingName == country &&
  //               name.toLowerCase() &&
  //               existingPhone == phone.replaceAll(RegExp(r'\D'), '');
  //         }) ??
  //         false;
  //     if (isDuplicate) {
  //       const successMessage = "Contact already exists";
  //       showErrorDialog(message: successMessage);
  //       return; // Stop here
  //     }
  //     // Check max 8 contacts
  //     if ((sosList?.length ?? 0) >= 8) {
  //       const successMessage = "Maximum emergency contacts limit reached";
  //       showErrorDialog(message: successMessage);
  //       return;
  //     }
