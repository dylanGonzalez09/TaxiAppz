...
import { getSubscriptionByPagination } from '@apis/subscription';
...
import { deleteByPackageId, getPackageByPagination, updatePackageStatus } from '@/app/api/apps/taxi/package';
...
const SubScriptionTable = ({ subScriptionData, dictionary }: { subScriptionData: any, dictionary: any,}) => {
  ...
  const handleConfirmStatus = async (confirmed: boolean) => {
    if (confirmed && statusSubScription) {
      ...
      try {
        await updatePackageStatus(statusSubScription.id.toString(), { status: updatedSubScription.status });
  ...
  const columns = useMemo<ColumnDef<SubScriptionWithActionsType, any>[]>(
    () => [
      ...
      columnHelper.accessor('name', {
        header: dictionary['navigation'].packageName,
        cell: ({ row }) => <Typography>{row.original.name}</Typography>
      }),
      columnHelper.accessor('noOfDrivers', {
        header: dictionary['navigation'].noofdrivers,
        cell: ({ row }) => <Typography>{row.original.noOfDrivers}</Typography>
      }),
      columnHelper.accessor('noOfUsers', {
        header: dictionary['navigation'].noofusers,
        cell: ({ row }) => <Typography>{row.original.noOfUsers}</Typography>
      }),
      columnHelper.accessor('type', {
        header: dictionary['navigation'].type,
        cell: ({ row }) => <Typography>{formatValidityPeriod(row.original.type)}</Typography>
      }),
      columnHelper.accessor('validityPeriod', {
        header: dictionary['navigation'].contractperiod,
        cell: ({ row }) => <Typography>{formatValidityPeriod(row.original.validityPeriod)}</Typography>
      }),
              ...
              options={[
                {
                  ...
                  menuItemProps: {
                    onClick: () => handleEditClick(row.original),
                      ...
                      {
              text: dictionary['navigation'].View,
              icon: 'tabler-script',
              href: getLocalizedUrl(`/apps/taxi/package/view/${row.original.id}`, locale as Locale),
              linkProps: {
  ...
  const handleConfirmDelete = async (confirmed: boolean) => {
    if (confirmed && deleteSubscriptionId) {
      try {
        await deleteByPackageId(deleteSubscriptionId);
  ...
  const handleSearch = useCallback(
    async (searchTerm: string) => {
      try {
        const result = await getPackageByPagination(
          searchTerm,
        ...
        <div className='flex flex-wrap justify-between gap-4 p-6'>
          ...
          <div className='flex items-center gap-x-4'>
            ...
            />
              ...
              >
                {dictionary['navigation'].addPackage}
              </Button>
