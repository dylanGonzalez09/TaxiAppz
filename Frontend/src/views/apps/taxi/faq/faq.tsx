...
import { deleteByFaqId, getByFaqByPagination, updateFaqStatus } from '@apis/faq';
...
const FaqTable = ({ dictionary,
  ...
  const [pageIndex, setPageIndex] = useState(FaqData.page - 1);
  const [pageSearch, setPageSearch] = useState("");
  const [totalResults, setTotalResults] = useState(FaqData.totalResults); // To track the total number of records
  ...
  const handleConfirmDelete = async (confirmed: boolean) => {
    if (confirmed && deleteFaqId) {
      try {
        ...
        if (data.length != 1) {
          setData((prevData: any[]) => prevData.filter(question => question.id !== deleteFaqId));
  ...
  const handleConfirmStatus = async (confirmed: boolean) => {
    if (confirmed && statusFaq) {
      const updatedFaq = { ...statusFaq, status: !statusFaq.status };

      try {
        await updateFaqStatus(statusFaq.id.toString(), { status: updatedFaq.status });
        setData((prevData: any[]) => prevData.map(question => (question.id === statusFaq.id ? updatedFaq : question)));
  ...
  const handlePageChange = async (event: unknown, newPage: number) => {
    try {
      // const { results, totalResults } = await getByFaqByPagination(pageSearch, newPage, pageSize);
  ...
  const handlePageSizeChange = async (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    // const { results, totalResults } = await getByFaqByPagination(pageSearch, 1, newPageSize);
  ...
  const handleSearch = useCallback(
    async (searchTerm: string) => {
      try {
        // const result = await getByFaqByPagination(
        //   searchTerm,
      ...
      <Card>
        ...
        <TablePagination
          component={() => <TablePaginationComponent
            ...
            totalResults={FaqData.totalResults}  // Pass the total results
