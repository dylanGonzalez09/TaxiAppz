...
const FineTable =(props:FineTabProps) => {
  const { userId,addFineButton,dictionary,fineData } = props;
  const [data, setData] = useState<FineType[]>(fineData || []);
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('requestNumber', {
        ...
        cell: ({ row }) => (
            <Typography color='success.main'>{row.original.requestNumber}</Typography>
        ),
      }),
      ...
      //   cell: ({ row }) => (
      //       <Typography color='success.main'>{row.original.name}</Typography>
      //   ),
      // }),
      ...
      <CardHeader
        title={
          <div className='flex justify-between items-center'>
            <span>{dictionary['navigation'].FineList}</span>
            {addFineButton && (
            <Button
              variant='contained'
