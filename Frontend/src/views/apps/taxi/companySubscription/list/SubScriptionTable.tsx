...
import { deleteByCompanySubScriptionId, getCompanySubscriptionByPagination, updateCompanySubScriptionStatus } from '@/app/api/apps/taxi/companySubscription';
...
export type subscriptionType = {
  ...
  noOfUsers: string;
  status: boolean;
  unit?: string; // Assuming unit is optional, adjust as necessary
  amount?: string; // Assuming amount is optional, adjust as necessary
};
...
const SubScriptionTable = ({ subScriptionData, dictionary }: { subScriptionData: any, dictionary: any }) => {
  ...
  const handleConfirmStatus = async (confirmed: boolean) => {
    if (confirmed && statusSubScription) {
      ...
      try {
        await updateCompanySubScriptionStatus(statusSubScription.id.toString(), { status: updatedSubScription.status });
  ...
  const handleConfirmDelete = async (confirmed: boolean) => {
    if (confirmed && deleteSubscriptionId) {
      try {
        await deleteByCompanySubScriptionId(deleteSubscriptionId);
  ...
  const handlePageChange = async (event: unknown, newPage: number) => {
    try {
      const { results, totalResults } = await getCompanySubscriptionByPagination(pageSearch, newPage, pageSize);
  ...
  const handlePageSizeChange = async (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const { results, totalResults } = await getCompanySubscriptionByPagination(pageSearch, 1, newPageSize);
  ...
  const handleSearch = useCallback(
    async (searchTerm: string) => {
      try {
        const result = await getCompanySubscriptionByPagination(
          searchTerm,
