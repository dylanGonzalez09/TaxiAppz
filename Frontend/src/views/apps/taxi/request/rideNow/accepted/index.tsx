...
const AcceptedTab = ({ AcceptedData, dictionary }: { AcceptedData?: any; dictionary: any }) => {
  ...
  const handlePageChange = async (event: unknown, newPage: number) => {
    try {
      const { results, totalResults } = await getRequestWithPagination(pageSearch, newPage, pageSize, "RIDE_NOW", "isDriverStarted");
  ...
  const handlePageSizeChange = async (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const { results, totalResults } = await getRequestWithPagination(pageSearch, 1, newPageSize, "RIDE_NOW", "isDriverStarted");
    ...
    async (searchTerm: string) => {
      try {
        const result = await getRequestWithPagination(
          searchTerm,
          1,
          pageSize, "RIDE_NOW", "isDriverStarted"
        );
