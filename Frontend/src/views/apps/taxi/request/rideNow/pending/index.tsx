...
const PendingTab = ({ PendingData, dictionary }: { PendingData?: any; dictionary: any }) => {
  ...
  const [pageIndex, setPageIndex] = useState(PendingData.page || 0);
  const [pageSearch, setPageSearch] = useState("");
  const [totalResults, setTotalResults] = useState(PendingData.totalResults || 0);
  const [data, setData] = useState(PendingData.results || []);
  const [pageSize, setPageSize] = useState(PendingData.limit || 10);
  const currentReturnPage = pageIndex;
  const returnQuery = `?returnModule=rideNow&returnTab=pending&returnPage=${currentReturnPage}&returnPageSize=${pageSize}&returnSearch=${encodeURIComponent(pageSearch)}`;
...
const [startDate, setStartDate] = useState<Date | null>();
  ...
  useEffect(() => {
    ...
    setTotalResults(PendingData?.totalResults ?? 0)
    setPageIndex(PendingData?.page ?? 0)
...
const handleChatHistoryClick = async (requestId: string) => {
  ...
  ], [zoneIdString,dictionary, locale, pageIndex, pageSize, formattedStartDate, formattedEndDate, paymentOpt]);
  ...
  const handlePageChange = async (event: unknown, newPage: number) => {
    try {
      const { results, totalResults } = await getRequestWithPagination(pageSearch, newPage, pageSize, "RIDE_NOW", "",paymentOpt,formattedStartDate,formattedEndDate,zoneIdString);
  ...
  const handlePageSizeChange = async (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const { results, totalResults } = await getRequestWithPagination(pageSearch, 1, newPageSize, "RIDE_NOW", "",paymentOpt,formattedStartDate,formattedEndDate,zoneIdString);
    ...
    async (searchTerm: string) => {
      try {
        const result = await getRequestWithPagination(
          searchTerm,
          1,
          pageSize, "RIDE_NOW", "",paymentOpt,formattedStartDate,formattedEndDate,zoneIdString
        );
