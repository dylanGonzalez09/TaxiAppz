...
const PendingTab = ({ PendingData, dictionary }: { PendingData?: any; dictionary: any }) => {
  ...
  const [pageIndex, setPageIndex] = useState(PendingData.page - 1);
  const [pageSearch, setPageSearch] = useState("");
  const [totalResults, setTotalResults] = useState(PendingData.totalResults);
  const [data, setData] = useState(PendingData.results);
  const [pageSize, setPageSize] = useState(PendingData.limit);
  ...
  const handlePageChange = async (event: unknown, newPage: number) => {
    try {
      const { results, totalResults } = await getRequestWithPagination(pageSearch, newPage, pageSize, "RIDE_LATER", "");
  ...
  const handlePageSizeChange = async (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const { results, totalResults } = await getRequestWithPagination(pageSearch, 1, newPageSize, "RIDE_LATER", "");
