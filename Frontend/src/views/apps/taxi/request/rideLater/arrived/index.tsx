...
const ArrivedTab = ({ ArrivedData, dictionary }: { ArrivedData?: any; dictionary: any }) => {
  ...
  const [totalResults, setTotalResults] = useState(ArrivedData.totalResults); // To track the total number of records
  ...
  const handlePageChange = async (event: unknown, newPage: number) => {
    try {
      const { results, totalResults } = await getRequestWithPagination(pageSearch, newPage, pageSize, "RIDE_LATER", "isDriverArrived");
  ...
  const handlePageSizeChange = async (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const { results, totalResults } = await getRequestWithPagination(pageSearch, 1, newPageSize, "RIDE_LATER", "isDriverArrived");
    ...
    async (searchTerm: string) => {
      try {
        const result = await getRequestWithPagination(
          ...
          pageSize, "RIDE_LATER", "isDriverArrived"
        );
