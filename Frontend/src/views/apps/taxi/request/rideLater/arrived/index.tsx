...
const ArrivedTab = ({ ArrivedData, dictionary }: { ArrivedData?: any; dictionary: any }) => {
  ...
  const [totalResults, setTotalResults] = useState(ArrivedData.totalResults || 0); // To track the total number of records
  ...
  const returnQuery = `?returnModule=rideLater&returnTab=arrived&returnPage=${currentReturnPage}&returnPageSize=${pageSize}&returnSearch=${encodeURIComponent(pageSearch)}`;
...
const handleChatHistoryClick = async (requestId: string) => {
  ...
  const handlePageChange = async (event: unknown, newPage: number) => {
    try {
      const { results, totalResults } = await getRequestWithPagination(pageSearch, newPage, pageSize, "RIDE_LATER", "isDriverArrived","All",formattedStartDate,formattedEndDate,zoneIdString);
  ...
  const handlePageSizeChange = async (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const { results, totalResults } = await getRequestWithPagination(pageSearch, 1, newPageSize, "RIDE_LATER", "isDriverArrived","All",formattedStartDate,formattedEndDate,zoneIdString);
    ...
    async (searchTerm: string) => {
      try {
        const result = await getRequestWithPagination(
          ...
          pageSize, "RIDE_LATER", "isDriverArrived","All",formattedStartDate,formattedEndDate,zoneIdString
        );
