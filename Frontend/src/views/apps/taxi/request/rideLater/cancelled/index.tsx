...
const CancelledTab = ({ CancelledData, dictionary }: { CancelledData?: any; dictionary: any }) => {
  const [rowSelection, setRowSelection] = useState({});
  const { lang: locale,zoneId } = useParams();
  const zoneIdString = Array.isArray(zoneId) ? zoneId[0] : zoneId;

  const [globalFilter, setGlobalFilter] = useState('');
  const [pageIndex, setPageIndex] = useState(CancelledData.page || 0);
  const [pageSearch, setPageSearch] = useState("");
  const [totalResults, setTotalResults] = useState(CancelledData.totalResults || 0);
  const [data, setData] = useState(CancelledData.results || []);
  const [pageSize, setPageSize] = useState(CancelledData.limit || 10);
  const currentReturnPage = pageIndex;
  const returnQuery = `?returnModule=rideLater&returnTab=cancelled&returnPage=${currentReturnPage}&returnPageSize=${pageSize}&returnSearch=${encodeURIComponent(pageSearch)}`;
...
const [startDate, setStartDate] = useState<Date | null>();
  ...
  useEffect(() => {
    setData(CancelledData?.results ?? [])
    setTotalResults(CancelledData?.totalResults ?? 0)
    setPageIndex(CancelledData?.page ?? 0)
    setPageSize(CancelledData?.limit ?? 10)
  }, [CancelledData])
...
const handleChatHistoryClick = async (requestId: string) => {
  ...
  ], [zoneIdString,dictionary, locale, pageIndex, pageSize, formattedStartDate, formattedEndDate]);
  ...
  const handlePageChange = async (event: unknown, newPage: number) => {
    try {
      const { results, totalResults } = await getRequestWithPagination(pageSearch, newPage, pageSize, "RIDE_LATER", "isCancelled","All",formattedStartDate,formattedEndDate,zoneIdString);
  ...
  const handlePageSizeChange = async (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const { results, totalResults } = await getRequestWithPagination(pageSearch, 1, newPageSize, "RIDE_LATER", "isCancelled","All",formattedStartDate,formattedEndDate,zoneIdString);
    ...
    async (searchTerm: string) => {
      try {
        const result = await getRequestWithPagination(
          searchTerm,
          1,
          pageSize, "RIDE_LATER", "isCancelled","All",formattedStartDate,formattedEndDate,zoneIdString
        );
