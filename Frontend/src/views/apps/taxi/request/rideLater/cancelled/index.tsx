...
const CancelledTab = ({ CancelledData, dictionary }: { CancelledData?: any; dictionary: any }) => {
  ...
  const [pageIndex, setPageIndex] = useState(CancelledData.page - 1);
  const [pageSearch, setPageSearch] = useState("");
  const [totalResults, setTotalResults] = useState(CancelledData.totalResults);
  const [data, setData] = useState(CancelledData.results);
  const [pageSize, setPageSize] = useState(CancelledData.limit);
  ...
  const handlePageChange = async (event: unknown, newPage: number) => {
    try {
      const { results, totalResults } = await getRequestWithPagination(pageSearch, newPage, pageSize, "RIDE_NOW", "isCancelled");
  ...
  const handlePageSizeChange = async (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const { results, totalResults } = await getRequestWithPagination(pageSearch, 1, newPageSize, "RIDE_NOW", "isCancelled");
