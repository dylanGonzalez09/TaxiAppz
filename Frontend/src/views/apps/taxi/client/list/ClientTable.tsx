...
import { deleteByClientId, getClientByPagination } from '@/app/api/apps/taxi/client';
...
return;
    ...
    setdeleteClientId(clientId.toString());
  ...
  const handleStatusToggle = async (client: ClientType) => {
    setstatusClient(client);
    setStatusConfirmationOpen(true);
  };

  const handleConfirmStatus = async (confirmed: boolean) => {
    if (confirmed && statusClient) {
      const updatedClient = { ...statusClient, status: !statusClient.status };

      try {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars

        // Dummy implementation for status update
        setData((prevData: { id: string }[]) =>
          prevData.map((client: { id: string | undefined; }) =>
            client.id === statusClient.id ? updatedClient : client
          )
        );
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            ...
            <OptionMenu
              ...
              options={[
                {
                  text: dictionary['navigation'].Push,
                ...
                {
                  text: dictionary['navigation'].Country,
                  icon: 'tabler-world',
                ...
                {
                  text: dictionary['navigation'].Language,
      ...
      <Card>
        <div className='flex flex-wrap justify-between gap-4 p-6'>
          <div className='flex items-center gap-x-4'>
            <CustomTextField
              variant="outlined"
              placeholder={dictionary['navigation'].searchhear}
              value={globalFilter}
              onChange={(e) => {
