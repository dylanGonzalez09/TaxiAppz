...
import { deleteByGroupDocumentById, updateGroupDocumentStatus, getGroupDocumentByPagination } from '@apis/groupDocument';

import type { GroupDocumentType } from '@/types/apps/masterType'; // Import the type
import AddGroupDocumentDialog from './AddEditDrawer';
...
const fuzzyFilter: FilterFn<GroupDocumentType> = (row, columnId, filterValue) => {
...
const GroupDocumentTable = ({ staticGroup, dictionary }: { staticGroup: any, dictionary: any }) => {
  ...
  const [statusGroupDocument, setStatusGroupDocument] = useState<GroupDocumentType | null>(null);
  ...
  const [data, setData] = useState<GroupDocumentType[]>(staticGroup.results);
  ...
  const handleStatusToggle = async (grouoDocument: GroupDocumentType) => {
    ...
    setStatusGroupDocument(grouoDocument);
    setStatusConfirmationOpen(true);
  };

  const handleConfirmStatus = async (confirmed: boolean) => {
    if (confirmed && statusGroupDocument) {
      const updatedGroupDocument = { ...statusGroupDocument, status: !statusGroupDocument.status };

      try {
      const response =  await updateGroupDocumentStatus(statusGroupDocument.id.toString(), { status: updatedGroupDocument.status });
      ...
      if (response.status === httpStatus.FORBIDDEN) {
       ...
       setData(prevData => {
          return prevData.map(groupDocument =>
            groupDocument.id === statusGroupDocument.id ? updatedGroupDocument : groupDocument
          );
        });
  ...
  const columns = useMemo<ColumnDef<GroupDocumentType, any>[]>(
    () => [
      {
        id: 'serialNo',
  ...
  const table = useReactTable({
    ...
    globalFilterFn: fuzzyFilter as FilterFn<GroupDocumentType>, // Cast fuzzyFilter to FilterFn<GroupDocumentType>
  ...
  const handleConfirmDelete = async (confirmed: boolean) => {
    if (confirmed && deleteGroupDocumentId) {
      try
      {
        const response = await deleteByGroupDocumentById(deleteGroupDocumentId);
        ...
        toast.error(dictionary['navigation'].Anerroroccurredwhiledeletingthegroupdocument);
      }
    }
    else
    {
  ...
  return (
    <>
        ...
        <div className='flex flex-wrap justify-between gap-4 p-6'>
          ...
          <div className='flex items-center gap-x-4'>
            ...
            <ExportOptions
              ...
              fileName='GroupDocument_Export'
              dictionary={dictionary}

            />
              <Button
                variant='contained'
                onClick={() => setAddGroupDocumentOpen(true)}
      ...
      <AddGroupDocumentDialog
        open={addGroupDocumentOpen}
        handleClose={handleCloseDialog}
        groupDocumentData={data}
        dictionary={dictionary}
        setData={setData}
        editData={editData}
        count={totalResults}
