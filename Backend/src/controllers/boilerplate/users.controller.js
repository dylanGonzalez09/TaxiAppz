...
const getDropDownList = catchAsync(async (req, res) => {
  const data = await usersService.getDropDowns(req.params.clientId);
...
const getDashboardCount = catchAsync(async (req, res) => {
  ...
  const data = await usersService.getDashboardByCount(clientId, zoneId);
...
const getDriverProfileDetails = catchAsync(async (req, res) => {
  const { userId } = req.params;
  ...
  // Fetch user details
  const driver = await usersService.getByDriverDetails(userId);
...
const getLogisticalCounts = async (req, res) => {
  ...
  if (!req.headers.zoneid) {
    throw new ApiError(httpStatus.NOT_FOUND, 'ZoenId not found');
  } else {
    zoneId = req.headers.zoneid;
  }

  const data = await usersService.getLogisticalCounts(clientId, zoneId);
