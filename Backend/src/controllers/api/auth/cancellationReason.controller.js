...
const getCancellations = catchAsync(async (req, res) => {
  const filter = pick(req.query, ['reason', 'userType', 'tripStatus', 'payStatus', 'role']);
  ...
  const result = await cancellationReasonService.queryCancellation(filter, options);
...
const getCancellationWithoutPagination = catchAsync(async (req, res) => {
  const cancellation = await cancellationReasonService.getDriverCancellations(req);
  if (!cancellation) {
...
const getUserCancellationWithoutPagination = catchAsync(async (req, res) => {
  const cancellation = await cancellationReasonService.getUserCancellations(req);
  if (!cancellation) {
    throw new ApiError(httpStatus.NOT_FOUND, 'cancellation not found');
  }
  const response = Response(true, cancellation, 'Success');
