...
const createVehicleModel = catchAsync(async (req, res) => {
  ...
  const { modelname, vehicleId, brandId, status } = req.body;
...
const updateVehicleModel = catchAsync(async (req, res) => {
  ...
  vehicleModelUpload.single('image')(req, res, async (err) => {
    ...
    const updateData = { modelname, vehicleId, brandId, status };
    if (req.headers.zoneid) {
      updateData.zoneId = req.headers.zoneid;
    }
...
const getDropDownList = catchAsync(async (req, res) => {
  const data = await vehicleModelService.getDropDowns(req.params.clientId, req.headers.zoneid || null);
