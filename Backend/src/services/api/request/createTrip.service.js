...
const getDispatchAdminId = async (req) => {
  let userId = '';
  const authHeader = req.headers.authorization;
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    res.status(httpStatus.UNAUTHORIZED).send({ message: 'Authorization header is missing or invalid' });
...
const validateUserRequestedTrip = async (user) => {
  try {
    ...
    if (requestMetaWithCurrentUser) {
      ...
      if (requestId) {
        await Request.update({ isCancelled: true, cancelMethod: 1 }, { where: { id: requestId } });
      }
...
const createTrip = async (req) => {
  ...
  if (req.body.trip_type == 'LOCAL') {
    ...
    req.body.payment_opt = 'CASH'; // Ensure trip type is set
...
const createTrips = async (req) => {
  ...
  const [clientId, userId] = await Promise.all([getClientId(req), getUserId(req)]);
  ...
  await Promise.all([validateUserInTrip(user), validateUserRequestedTrip(user)]);
...
const createBiddingTrips = async (req) => {
  // Parallelize initial data fetching
  const [clientId, userId] = await Promise.all([getClientId(req), getUserId(req)]);
  ...
  const { trip_type } = req.body;
  if (trip_type === 'LOCAL') {
    return await createBiddingLocalTrip(req, user, clientId);
  } else if (trip_type === 'RENTAL') {
    return await createBiddingRentalTrip(req, user, clientId);
  } else {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Invalid trip type');
  }
};
...
const createLocalTrip = async (req, user, clientId) => {
  ...
  const [type, zone] = await Promise.all([Vehicle.findOne({ _id: vehicle_type, clientId }), getPickupZone(req)]);
  ...
  // Find zone price
  const zonePriceDetail = zone.zonePriceDetails.find((zp) => zp.vehicleDetails._id.equals(type._id));
  ...
  const surgePriceDetail = zone.zoneSurgePriceDetails.find((surge) => {
    const startTime = moment(surge.startTime, 'HH:mm');
    const endTime = moment(surge.endTime, 'HH:mm');
    return currentTime.isBetween(startTime, endTime) && surge.availableDays.includes(today);
  });

  // Validate payment option
  if (payment_opt) {
    const isAvailable = zone.paymentTypes.some((opt) => opt.toLowerCase() === payment_opt.toLowerCase());
  ...
  const [requestNumber, requestOtp] = await Promise.all([generateRequestNumber(), uniqueRandomNumbers(4)]);
  ...
  if (stops) {
    ...
    stopLocations.forEach(async (currentStop, i) => {
      const prevStop = stopLocations[i - 1] || {
        latitude: pick_lat,
        longitude: pick_lng,
        address: req.body.pick_address,
      };
  ...
  if (!isLater) {
    return await assignDriverAndNotify(
      requestDetail,
      pick_lat,
      pick_lng,
      vehicle_type,
      requestDetail.tripType,
      zone._id,
      drop_lat,
      drop_lng,
      clientId
...
const createRentalTrip = async (req, user, clientId) => {
  const { promo_code, booking_for, payment_opt, pick_lat, pick_lng, vehicle_type, ride_type, isLater, packageId } = req.body;
...
const assignDriverAndNotify = async (
  requestDetail,
  pick_lat,
  pick_lng,
  vehicle_type,
  ...
  try {
    ...
    const driverAvailabilityChecks = drivers.map(async (driver) => {
      const activeTripCount = await RequestMeta.countDocuments(
        {
          driverId: driver.driverId,
          active: true, // ✅ BOOLEAN ONLY
        },
        { session }
      );
    ...
    const driverTopic =
      mqttConfig.DRIVER_REQUEST + String(firstDriver.driverId);

    // Use Promise.allSettled to prevent one failure from blocking the API response
    ...
    await RequestDriverData.create(
      [
        {
          requestId: requestDetail.id,
          driverIds: [firstDriver.driverId],
        },
      ],
      { session }
    );
...
const assignDispatchDriverAndNotify = async (
  adminDemoKey,
  requestDetail,
  pick_lat,
  pick_lng,
  ...
  try {
    ...
    const drivers = await fetchDispatchDriver(
      adminDemoKey,
      pick_lat,
      pick_lng,
      vehicle_type,
...
// Helper function for LOCAL trips Bidding

const createBiddingLocalTrip = async (req, user, clientId) => {
  const {
    promo_code,
    booking_for,
    payment_opt,
  ...
  if (!isLater) {
    return await assignBiddingDriverAndNotify(
      requestDetail,
      pick_lat,
      pick_lng,
      vehicle_type,
...
const createBiddingRentalTrip = async (req, user, clientId) => {
  const {
    promo_code,
    booking_for,
    payment_opt,
...
const assignBiddingDriverAndNotify = async (
  requestDetail,
  pick_lat,
  pick_lng,
  vehicle_type,
  ...
  try {
    ...
    const responseData = await getRequestListData(request.id);

    await sendNotificationsForDrivers(drivers, promoAmount, request, responseData[0], estAmount);
...
const sendNotificationsForDrivers = async (drivers, promoAmount, requestDetail, responseData, estAmount) => {
  // Extract driver IDs
  ...
  const existingBids = await RequestBid.find({
    ...
    tripType: requestDetail.tripType,
  });
  ...
  const notificationTasks = driversData.map(async (driver) => {
    ...
    return await Promise.all([
      ...
      mqttService.publishMessage(driverTopic, message),
    ]);
  });
...
// End Create Trip From Mobile Bidding

const createDispatcher = async (req) => {
  let clientId = await getClientId(req);
  let promocode_id = 0;

  const { userId, promo_code, booking_for, payment_opt, pick_lat, pick_lng, drop_lat, drop_lng, stops, driver_notes, vehicle_type, ride_type, trip_type, driverId, manual_trip, passengerName, passengerNumber, ride_date,etaAmount } = req.body;
  ...
  if(trip_type === 'RENTAL')
  {
    return await createRentalDispatcher(req,user,zone);
  }
  ...
  //chk if admin is demo
  const adminId = await getDispatchAdminId(req);
  const adminDemoKey = await findDemoOrNot(adminId);

  //for manual trip chk driver
  let driver_id = null;
  let holdStatus = 0;
  if (driverId && manual_trip === 'MANUAL') {
    const getDriver = await Driver.findById(driverId);
    if (!getDriver) {
      throw new ApiError(httpStatus.NOT_FOUND, 'Selected Driver not found');
    }
    driver_id = getDriver._id;
    holdStatus = 1;
  }
  //  console.log
  // Check for surge pricing
  ...
  if (req.body.payment_opt) {
    try {
      // Check if zone has paymentTypes defined
      if (!zone?.paymentTypes || !Array.isArray(zone.paymentTypes)) {
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Payment options not configured for this zone');
      }

      const paymentOptions = zone.paymentTypes;
      const requestedPaymentOption = req.body.payment_opt;

      // Case-insensitive comparison if needed (optional)
      const normalizedOptions = paymentOptions.map((opt) => opt.toLowerCase());
      const normalizedRequest = requestedPaymentOption.toLowerCase();

      if (!normalizedOptions.includes(normalizedRequest)) {
        // Provide available options in the error message for better UX
        throw new ApiError(
          httpStatus.FORBIDDEN,
          `Payment option "${requestedPaymentOption}" is not available. ` +
            `Please select from: ${paymentOptions.join(', ')}`
        );
      }

      // Optional: Add additional validation for payment method
      if (requestedPaymentOption === 'credit_card' && !req.body.card_token) {
        throw new ApiError(httpStatus.BAD_REQUEST, 'Card token is required for credit card payments');
      }

      // If everything is valid, attach the validated payment option to the request
      req.validatedPaymentOption = requestedPaymentOption;
    } catch (error) {
      // Pass through ApiError, wrap other errors
      if (error instanceof ApiError) {
        throw error;
      }
      throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Payment option validation failed', error.message);
    }
  }
  ...
  const requestParams = {
    ...
    promoId,
    unit: zone.unit, // Add logic to get unit dynamically
    requestedCurrencyCode: countryDial.currency_code, // Placeholder
    ...
    adminDemoKey,
    holdStatus,
    manualTrip: manual_trip,
    tripTime: isLater ? ride_date : null,
    zoneId: zone._id,
    etaAmount: etaAmount || 0,
  };
  ...
  if (stops) {
    ...
    placePromises.push(RequestPlace.create({
      ...
      dropLat: drop_lat,
      dropLng: drop_lng,
      stopLat: stopLocations.latitude,
      stopLng: stopLocations.longitude,
      stopAddress: stopLocations.address,
      pickAddress: req.body.pick_address,
      dropAddress: req.body.drop_address,
      requestId: requestDetail._id,
      stops: 1,
  ...
  if (manual_trip === "MANUAL") {
    if(ride_type === 'RIDE_NOW')
    {
      await RequestMeta.deleteMany({requestId: requestDetail.id});
      ...
      const driverInUser = await Users.findById(driver.userId);

      sendPushNotification(driverInUser._id, {
        title: "New Trip Requested",
        message: "New Trip Requested, you can accept or Reject the request"
      });
    ...
    else
    {
     ...
     if(ride_type === 'RIDE_NOW')
    {
      return await assignDispatchDriverAndNotify(adminDemoKey,requestDetail, pick_lat, pick_lng, vehicle_type, requestDetail.tripType, zone._id, drop_lat, drop_lng);
    }
    else
    {
      return requestDetail;
...
const createRentalDispatcher = async (req, user, zone) => {

  // const { promo_code, booking_for, payment_opt, pick_lat, pick_lng, vehicle_type, ride_type, manual_trip, trip_type, packageId } = req.body;
  const { userId, promo_code, booking_for, payment_opt, pick_lat, pick_lng, vehicle_type, ride_type, trip_type, driverId, manual_trip, packageId, ride_date } = req.body;
  ...
  const type = await Vehicle.findOne({ _id: vehicle_type });
 ...
 //chk if admin is demo
      ...
      try {
        ...
        if (!zone?.paymentTypes || !Array.isArray(zone.paymentTypes)) {
          throw new ApiError(
            ...
            'Payment options not configured for this zone'
          );
        }
        ...
        throw new ApiError(
          httpStatus.INTERNAL_SERVER_ERROR,
          'Payment option validation failed',
          error.message
        );
      }
    }
  ...
  if (manual_trip === "MANUAL") {
    ...
    if (ride_type === 'RIDE_NOW') {
      return await assignDispatchDriverAndNotify(adminDemoKey,requestDetail, pick_lat, pick_lng, vehicle_type, requestDetail.tripType, zone._id, "", "");
    }
    else {
      return requestDetail;
    }
...
const getRequestpagination = async (req, filter, options) => {
  ...
  try {
    const results = await Request.aggregate([
      ...
      {
        $addFields: {
          billingDetails: { $arrayElemAt: ['$billingDetails', 0] },
          placesDetails: { $arrayElemAt: ['$placesDetails', 0] },
...
const getOtp = async () => {
  const driverOtpType = await Settings.findOne({ name: 'driverOtpType' });
  let requestOtp = null;
  if (driverOtpType && driverOtpType.value === 'dynamic') {
    requestOtp = await uniqueRandomNumbers(4);
  }
