const { Request, RequestPlace, Driver, requestListView } = require('../../../models');
...
const startTrips = async (req) => {
  try {
    ...
    const [tripRequest, driver] = await Promise.all([Request.findById(requestId), Driver.findById(driverId)]);
    ...
    const [responseData] = await Promise.all([getRequestListData(requestId)]);
