    ...
    if (user.onlineBy === 1) {
      ...
      if (log) {
        ...
        const workingHours = moment
          .utc(moment.duration(offlineTime.diff(onlineTime)).asMilliseconds())
          .format('HH:mm:ss');
...
const getDriversBytoken = async (token) => {
  ...
  let driverId = await getTokenUserId(token);
...
const updateDriverStatusByDemo = async(req) =>{
  let driverId = await getDriverrId(req);

  let userDetails = await Driver.findById(driverId);
