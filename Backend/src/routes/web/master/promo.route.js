...
const { promoUpload } = require('../../../middlewares/upload');
...
router
  .route('/getPromoCodes/:promoCodeId')
  .get(auth('PromoCode'), validate(promoCodeValidation.getPromoCode), promoCodeController.getPromoCode);
router
  .route('/updatePromoCodes/:promoCodeId')
  .patch(
    auth('PromoCode'),
...
router
  .route('/deletePromoCodes/:promoCodeId')
  .delete(auth('PromoCode'), validate(promoCodeValidation.deletePromoCode), promoCodeController.deletePromoCode);
router
  .route('/getPromoCode')
  .get(auth('PromoCode'), validate(promoCodeValidation.getPromoCodes), promoCodeController.getPromosWithoutPagination);
router.patch(
  '/updatePromoCodeStatus/:promoCodeId',
  auth('PromoCode'),
  validate(promoCodeValidation.updatePromoStatus),
  promoCodeController.updatePromoStatus,
);
router.route('/getDropDown/list/:clientId').get(promoCodeController.getDropDownList);
router.route('/getPromoDropDown/list/:zoneId/:passengerNumber').get(promoCodeController.getPromoCodeDropDownList);
